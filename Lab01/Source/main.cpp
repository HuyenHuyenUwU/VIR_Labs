...
int main() {
    ...
    if (!engine.initialize(featureDatabaseDir.generic_string() + "/")) {
        std::cerr << "Loi: Khong tim thay hoac khong nạp duoc cac file db_*.yml!" << endl;
        std::cerr << "Vui long chay file training.cpp truoc de tao ra 7 file database." << std::endl;
        return -1;
    }
    ...
    while (true) {
        string queryPath;
        cout << "\nNhap duong dan anh query (vd: ../Dataset/test/01.jpg, (Hoac go 'exit' de thoat): ";

        cin >> queryPath;

        if (queryPath == "exit") break;

        cv::Mat queryImg = cv::imread(queryPath);
        if (queryImg.empty()) {
            cout << "Duong dan anh khong hop le hoặc file loi. Vui long nhap lai!" << endl;
            continue;
        }

        // Cấu hình tham số online: Tìm kiếm kết hợp 7 đặc trưng
        // Trọng số: blob, colorhistogram, colorcorrelogram, corner, sift, orb, texture
        // Tổng các trọng số nên = 1.0 để chuẩn hóa
        ...
        cout << "Dang so khop cac dac truong..." << endl;
        InferenceOutput output = engine.query(queryImg, weights, K);

        // Gọi cấu hình hiển thị cửa sổ giao diện đồ họa
        displayResultGrid(queryImg, output.results, output.executionTimeMs);
    }
